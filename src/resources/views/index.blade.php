...
@section('content')
    <div class="container-fluid">
        ...
        <div class="bg-white card shadow">
            <div class="card-body">
                @if(Spatie\Permission\Models\Permission::where('name', 'import-' . strtolower(\Illuminate\Support\Str::snake($modelName)))->exists())
                    @can('import-' . strtolower(\Illuminate\Support\Str::snake($modelName)))
                ...
                @else
                    <div class="d-flex">
                        @can('create-' . strtolower(\Illuminate\Support\Str::snake($modelName)))
                            <a href="{{ route($routePrefix . '.' . $routeName . '.create') }}" class="btn btn-primary rounded me-2"><i class="uil-plus-circle"></i> Create </a>
                        @endcan
                        @if(Spatie\Permission\Models\Permission::where('name', 'import-' . \Illuminate\Support\Str::snake(strtolower($modelName)))->exists())
                            @can('import-' . strtolower(\Illuminate\Support\Str::snake($modelName)))
                                <button class="btn btn-info rounded me-2" type="button" onclick="$('#file-upload').click()"><i class="uil-plus-circle"></i>  Import</button>
                            @endcan
                        @endif
                        @if(Spatie\Permission\Models\Permission::where('name', 'export-' . \Illuminate\Support\Str::snake(strtolower($modelName)))->exists())
                            @can('export-' . strtolower(\Illuminate\Support\Str::snake($modelName)))
