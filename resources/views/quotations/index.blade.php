            ...
            <div class="overflow-auto">
                <table class="table">
                    ...
                    @forelse ($models as $$routeModel)
                        <tr>
                            <td>Q #{{ str_pad($$routeModel->id, 5, '0', STR_PAD_LEFT) }}</td>
                            <td>{{ $$routeModel->name }}</td>
                            <td>{{ $$routeModel->delivery_date }}</td>
                            <td>{{ $$routeModel->client->name }}</td>
                            <td>$ {{ $$routeModel->grand_total }}</td>
                            <td>
                                {{ $$routeModel->status_text }}
                            </td>
                            <td>{{ $$routeModel->created_at }}</td>
                            <td>
                                @can('view-quotation')
                                    <a href="{{ route('admin.quotations.show', [$$routeModel->getRouteKey()]) }}"><i class="uil-eye mx-1"></i></a>
                                @endcan
                                @can('edit-quotation')
                                    <a href="{{ route('admin.quotations.edit', [$$routeModel->getRouteKey()]) }}"><i class="uil-edit"></i></a>
                                @endcan
                            </td>
                        </tr>
                    @empty
