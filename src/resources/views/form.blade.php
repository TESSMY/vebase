            ...
            <div class="col-12 {{ $updateField['size'] ?? 'col-md-6' }} mb-md-2 mb-2">
                ...
                @elseif ($updateField['inputType'] == 'file')
                        @if (!empty($$routeModel) && $$routeModel[$updateField['name']] && !empty($updateField['show']))
                           ...
                           @endif
                        @endif
                    <input class="form-control" type="{{ $updateField['inputType'] }}" {{ !empty($updateField['multiple']) ? 'multiple' : '' }} accept="{{ !empty($updateField['accept']) ? $updateField['accept'] : '*' }}" name="{{ $updateField['name'] }}{{ !empty($updateField['multiple']) ? '[]' : '' }}" {{ !empty($updateField['required']) ? 'required' : '' }}>
                @else
