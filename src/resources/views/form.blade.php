        ...
        @foreach ($model->updateFields as $updateField)
            <div class="col-12 {{ $updateField['size'] ?? 'col-md-6' }} mb-md-2 mb-2">
                ...
                @elseif ($updateField['inputType'] == 'file')
                    <input class="form-control" type="{{ $updateField['inputType'] }}" accept="{{ !empty($createField['accept']) ? $createField['accept'] : '*' }}" name="{{ $updateField['name'] }}" {{ !empty($updateField['required']) && $updateField['required'] == 'true' ? 'required' : '' }}>
                @else
