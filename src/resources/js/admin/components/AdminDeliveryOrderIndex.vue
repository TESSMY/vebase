                    ...
                    <tbody>
                        <template v-if="data.length">
                            <tr v-for="deliveryOrder in data">
                                ...
                                <td>
                                    <a v-if="parseInt(deliveryOrder.status) == 0" :href="`/admin/delivery-orders/${deliveryOrder.id}`" class="btn btn-sm btn-primary">View</a>
    ...
    <div class="modal fade" id="modal-detail" tabindex="-1" role="dialog" aria-labelledby="modalDetail" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <admin-delivery-order-summary v-if="deliveryOrderSelected" :key="deliveryOrderSelected.id" :delivery-order="deliveryOrderSelected"/>
            </div>
        </div>
    </div>
</template>
